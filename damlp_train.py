...
def train(source, model, optimizer, epoch):
    s_loader = tqdm(enumerate(source), total=len(source))
    len_dataloader = len(s_loader)
    loss_fin = 0
    y_pred_list, y_list = [], []

    for i, (xs1, xs2, ys, _) in s_loader:

        ys = ys.to(torch.int64)
        xs1, xs2, ys = xs1.to(my_device), xs2.to(my_device), ys.to(my_device)

        model.zero_grad()

        y_pred = model(xs1, xs2)
        loss = loss_cate(y_pred, torch.flatten(ys))
    ...
    loss_fin = loss_fin / len_dataloader
    ...
    print('EPOCH {} TRAINING SET RESULTS: Average loss: {:.4f} Accuracy: {:.4f}'
          .format(epoch, loss_fin, accuracy_fin))

    del xs1, xs2, ys, y_pred_list, y_list
...
def main(args):
    ...
    for epoch in range(1, epochs + 1):
        # source, target
        train(dl_s_tr, model, optimizer, epoch)
        # target
